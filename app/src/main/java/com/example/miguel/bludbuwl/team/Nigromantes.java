...
public class Nigromantes extends Equipo {
    ...
    public HashSet<Jugador> getJugadores() {
        if (jugadores.isEmpty()) {
            ...
            jugadores.add(new Jugador("Necrofagos", 70000, 7, 3, 3, 7, new HashSet<>(Arrays.asList(Habilidad.ESQUIVAR.getId())),2));
            ...
            jugadores.add(new Jugador("Golems de Carne", 110000, 4, 4, 2, 9, new HashSet<>(Arrays.asList(Habilidad.REGENERACION.getId(), Habilidad.MANTENERSEFIRME.getId(), Habilidad.CABEZADURA.getId())),2));
            jugadores.add(new Jugador("Hombres Lobo Nigrománticos", 120000, 8, 3, 3, 8, new HashSet<>(Arrays.asList(Habilidad.GARRAS.getId(), Habilidad.FURIA.getId(), Habilidad.REGENERACION.getId())),2));
